...
def feature_extract(prj_name, kernel_name, run_dir, out_dir, tb_dir):
    ...
    os.system('clang++ -fPIE -c {a}/{b}/ir_revise/annotated.bc -o {a}/{b}/act_trace/annotated.o'.format(a = out_dir, b = prj_name))
    os.system('/usr/bin/c++ -fopenmp -O3 -fPIC -std=c++11 -fPIE -o {a}/{b}/act_trace/main.o -c {c}/io{d}/main.cpp'.format(a = out_dir, b = prj_name, c = tb_dir, d = iob))
    os.system('/usr/bin/c++ -fopenmp -O3 -fPIC -std=c++11 -pie -o {a}/{b}/act_trace/act_trace {a}/{b}/act_trace/main.o {a}/{b}/act_trace/annotated.o {c}/act_trace/build/rtlop_tracer.o {c}/act_trace/build/tracer.o'.format(a=out_dir, b=prj_name, c=run_dir))
